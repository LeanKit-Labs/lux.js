	...
	constructor() {
			...
			states: {
				ready: {
					...
					"register.store" : function(storeMeta) {
						for(var action of storeMeta.actions) {
							var action;
							var actionName = action.actionType;
							var actionMeta = { namespace: storeMeta.namespace, waitFor: action.waitFor };
		...
		this.__subscriptions = [
			configSubscription(
				...
				luxCh.subscribe(
					"action",
					function( data, env ) { this.handleActionDispatch(data); }
				)
			),
			configSubscription(
				this,
				luxCh.subscribe(
					"register",
					function( data, env ) { this.handleStoreRegistration(data); }
				)
			)
		];
	}
